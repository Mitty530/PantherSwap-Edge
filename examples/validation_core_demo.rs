// Core data validation demo for PantherSwap Edge
// Demonstrates validation functionality using direct imports
// Run with: cargo run --example validation_core_demo

use pantherswap_edge::database::{
    types::*,
    validation::{DataValidator, ValidationConfig},
    data_quality::{DataQualityAssessor, QualityConfig},
};
use chrono::Utc;
use uuid::Uuid;
use serde_json::json;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🚀 PantherSwap Edge Core Data Validation Demo");
    println!("==============================================");
    
    // Initialize validation components
    let mut data_validator = DataValidator::with_default_config();
    let mut data_quality_assessor = DataQualityAssessor::with_default_config();
    ...
    // Test 5: Validation Configuration
    println!("\n⚙️  Testing Custom Validation Configuration...");
    
    // Create custom validation config with stricter rules
    let mut custom_config = ValidationConfig::default();
    custom_config.market_data.min_data_quality = 0.9; // Require 90% quality
    custom_config.trading_signals.min_confidence = 0.8; // Require 80% confidence
    
    let mut strict_validator = DataValidator::new(custom_config);
    
    // Test with medium quality data (should fail with strict config)
    let medium_quality_tick = MarketTick {
        timestamp: Utc::now(),
        instrument_id,
        provider: "alpha_vantage".to_string(),
        bid_price: 45000.50,
        ...
        volume: Some(100.0),
        spread: 1.00,
        data_quality_score: 0.75, // Below strict threshold
        raw_data: json!({"source": "demo", "quality": "medium"}),
    };

    match strict_validator.validate_market_tick(&medium_quality_tick) {
        Ok(()) => println!("⚠️  Medium quality tick unexpectedly passed strict validation"),
        Err(e) => {
            println!("✅ Strict validation correctly rejected medium quality data:");
            println!("   - Error: {}", e);
        }
    }
    ...
    println!("\n🎉 Core Data Validation Demo Completed Successfully!");
    println!("===================================================");
    ...
    println!("✅ Custom validation configuration working");
